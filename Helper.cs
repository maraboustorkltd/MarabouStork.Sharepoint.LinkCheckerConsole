...
namespace MarabouStork.Sharepoint.LinkCheckerConsole
{
    public class LinkChecker
    {
        ...
        ///     to invalid urls
        /// </summary>
        public static void ValidateDocumentUrls()
        {
            ValidateDocumentUrls(DateTime.MinValue); 
        }
        ...
        /// </param>
        public static void ValidateDocumentUrls(DateTime modsSinceDate)
        {
            baseSiteUrl = System.Configuration.ConfigurationManager.AppSettings["siteBaseUrl"];

            // Determine the list of document libraries to check
            string[] docLibs = System.Configuration.ConfigurationManager.AppSettings["docLibraries"].Split(new char[] { ',', ';' });
            ...
            List<String> fieldsToCheck = new List<string>(new string[] { "my:URL", "my:Description" });
            if (System.Configuration.ConfigurationManager.AppSettings["fieldsToCheck"] != null)
                fieldsToCheck = new List<string>(System.Configuration.ConfigurationManager.AppSettings["fieldsToCheck"].Split(new char[] { ',', ';' }));

            if (System.Configuration.ConfigurationManager.AppSettings["unpublishInvalidDocs"] != null)
            {
                bool unpublish;
                if (bool.TryParse(System.Configuration.ConfigurationManager.AppSettings["unpublishInvalidDocs"].ToString(), out unpublish))
                {
                    shouldUnpublish = unpublish;
                }
            }
